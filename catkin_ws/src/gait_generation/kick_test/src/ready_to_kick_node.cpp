...
{
	cout<<"Starting ready_to_kick by Luis Näva....."<<endl;
	...
	boost::shared_ptr<std_msgs::Bool const> kick_flag;
	...
	while(ros::ok())
	{
		...
		case SM_PUBLISH_PROFILES:
			...
			case SM_WAIT_FOR_COMMAND:
				cout<<"SM_WAIT_FOR_COMMAND"<<endl;
				kick_flag = ros::topic::waitForMessage<std_msgs::Bool>("/kick_test/kick_flag",nh);
				if(kick_flag != NULL)
					state = SM_PUBLISH_PROFILES;
				break;
